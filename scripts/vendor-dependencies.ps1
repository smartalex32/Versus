...
try {
    $generatedConfig = & cargo vendor --locked --versioned-dirs vendor
    if ($LASTEXITCODE -ne 0) { throw "cargo vendor failed with exit code $LASTEXITCODE" }
    @('# Generated by cargo vendor; keep the source replacement checked in.', $generatedConfig, '', '[net]', 'offline = true', '', '[target.x86_64-pc-windows-msvc]', 'rustflags = ["-C", "target-feature=+crt-static"]') | Set-Content -LiteralPath $cargoConfig -Encoding utf8
} catch {
